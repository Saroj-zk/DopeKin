...
<script>
  ...
  window.nameParam = window.urlParams.get('name') || 'Twin';
  
  // Safe extraction of avatar image and metadata from localStorage or window.name bridge
  window.imgUrl = 'Assets/Vale.jpeg';
  window.professionParam = window.urlParams.get('profession') || '';
  window.vibeParam = window.urlParams.get('vibe') || '';
  window.promptParam = window.urlParams.get('prompt') || '';

  try {
    if (window.name) {
      const storedTwin = JSON.parse(window.name);
      if (storedTwin && storedTwin.name === window.nameParam) {
        if (storedTwin.img) window.imgUrl = storedTwin.img;
        if (storedTwin.profession) window.professionParam = storedTwin.profession;
        if (storedTwin.vibe) window.vibeParam = storedTwin.vibe;
        if (storedTwin.prompt) window.promptParam = storedTwin.prompt;
      }
    }
  } catch (winErr) {
    console.warn("Could not read window.name bridge in chat:", winErr);
  }

  try {
    if (window.imgUrl === 'Assets/Vale.jpeg') {
      const activeTwin = JSON.parse(localStorage.getItem('activeTwin'));
      if (activeTwin && activeTwin.name === window.nameParam && activeTwin.img) {
        window.imgUrl = activeTwin.img;
      } else {
        const userTwins = JSON.parse(localStorage.getItem('userTwins') || '[]');
        const found = userTwins.find(t => t.name === window.nameParam);
        if (found && found.img) {
          window.imgUrl = found.img;
        } else {
          const urlImg = window.urlParams.get('img');
          if (urlImg) window.imgUrl = urlImg;
        }
      }
    }
  } catch (e) {
  ...
  // Extract subscription price from window.name or localStorage
  window.subscriptionPriceParam = 0;
  try {
    if (window.name) {
      const storedTwin = JSON.parse(window.name);
      if (storedTwin && storedTwin.name === window.nameParam && storedTwin.subscriptionPrice) {
        window.subscriptionPriceParam = storedTwin.subscriptionPrice;
      }
    }
    if (!window.subscriptionPriceParam) {
      const userTwins = JSON.parse(localStorage.getItem('userTwins') || '[]');
      const found = userTwins.find(t => t.name === window.nameParam);
      if (found && found.subscriptionPrice) {
        window.subscriptionPriceParam = found.subscriptionPrice;
      }
    }
  } catch (err) {
    console.warn("Could not load subscriptionPrice inside chat:", err);
  }

  // Guarantee window.name is fully populated so index.html syncs correctly
  try {
    window.name = JSON.stringify({
      name: window.nameParam,
      profession: window.professionParam,
      vibe: window.vibeParam,
      img: window.imgUrl,
      prompt: window.promptParam || '',
      fans: "0 FANS",
      subscriptionPrice: window.subscriptionPriceParam || 0
    });
    console.log("Successfully synced window.name bridge for:", window.nameParam);
  } catch (syncErr) {
    console.warn("Could not sync window.name back in chat:", syncErr);
  }
  ...
  document.getElementById('chatName').textContent = window.nameParam;
  document.getElementById('chatAvatar').style.backgroundImage = 'url("' + window.imgUrl + '")';
  ...
  // Initial greeting
  setTimeout(() => {
    chatHistory.innerHTML += '<div class="msg-bubble ai">Hey! I\'m ' + window.nameParam.split(' ')[0] + '. So glad you\'re here. What\'s on your mind today?</div>';
  }, 400);
